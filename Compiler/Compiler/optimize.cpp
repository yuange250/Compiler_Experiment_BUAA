...
void DAG_sblock_optimize(int fb_num,int sb_num)
{
	...
	for (int i = 0; i < fbs[fb_num].sbs[sb_num].codes_num; i++)
	{
		...
		{
			if (codes[i].opr == "ADD" || codes[i].opr == "MUL")
			{
				if (dnodes[it->second].text == codes[i].opr)
				{
					if ((
						ifin_array(codes[i].src1, dnodes[dnodes[it->second].left_child_num].vars, dnodes[dnodes[it->second].left_child_num].vars_num) && 
						ifin_array(codes[i].src2, dnodes[dnodes[it->second].right_child_num].vars, dnodes[dnodes[it->second].right_child_num].vars_num)) 
						|| (
						ifin_array(codes[i].src1, dnodes[dnodes[it->second].right_child_num].vars, dnodes[dnodes[it->second].right_child_num].vars_num) && 
						ifin_array(codes[i].src2, dnodes[dnodes[it->second].left_child_num].vars, dnodes[dnodes[it->second].left_child_num].vars_num)))
					{
						vars_map[codes[i].des] = it->second;
						node_no = it->second;
...
//						dnodes[dnodes[it->second].right_child_num].fathers_num++;
			...
			else if (codes[i].opr == "DIV" || codes[i].opr == "SUB")
			{
				if (dnodes[it->second].text == codes[i].opr)
				{
					if (ifin_array(codes[i].src1, dnodes[dnodes[it->second].left_child_num].vars, dnodes[dnodes[it->second].left_child_num].vars_num) &&
						ifin_array(codes[i].src2, dnodes[dnodes[it->second].right_child_num].vars, dnodes[dnodes[it->second].right_child_num].vars_num))
					{
						vars_map[codes[i].des] = it->second;
						node_no = it->second;
...
//						dnodes[dnodes[it->second].left_child_num].fathers_num++;
			...
			else if (codes[i].opr == "LOAD"&&store_flag)
			{
				if (dnodes[it->second].text == codes[i].opr)
				{
					if (ifin_array(codes[i].src1, dnodes[dnodes[it->second].left_child_num].vars, dnodes[dnodes[it->second].left_child_num].vars_num) &&
...
//						dnodes[dnodes[it->second].right_child_num].fathers_num++;
		...
		if (node_no == 0)
		{
			...
			{
				map<string, int>::iterator it = vars_map.find(codes[i].src1);
				if (it != vars_map.end())
				{
	...
	//				dnodes[it->second].fathers_num++;
				...
				{
					dnodes[nodes_num].text = codes[i].src1;
					...
					dnodes[nodes_num].vars[dnodes[nodes_num].vars_num++] = codes[i].src1;
					dnodes[nodes_num].fathers_num=0;
					left_child_no = nodes_num;
					vars_map[codes[i].src1] = nodes_num++;
				}
				/*
				...
				dnodes[nodes_num].vars_num = 0;
				*/
				dnodes[left_child_no].vars[dnodes[left_child_no].vars_num++] = codes[i].des;
				vars_map[codes[i].des] = left_child_no;
				...
				if (codes[i].src2 != "")
				{
					map<string, int>::iterator it2 = vars_map.find(codes[i].src2);
					if (it2 != vars_map.end())
					{
					...
					{
						dnodes[nodes_num].text = codes[i].src2;
						dnodes[nodes_num].left_child_num = 0;
						dnodes[nodes_num].right_child_num = 0;
						dnodes[nodes_num].vars_num = 0;
						dnodes[nodes_num].enable = true;
						dnodes[nodes_num].fathers_num=1;
						dnodes[nodes_num].vars[dnodes[nodes_num].vars_num++] = codes[i].src2;
						right_child_no = nodes_num;
						vars_map[codes[i].src2] = nodes_num++;
					}
				}
				else
					right_child_no = 0;
				...
				dnodes[nodes_num].enable = true;
				dnodes[nodes_num].fathers_num = 0;
				dnodes[nodes_num].vars[dnodes[nodes_num].vars_num++] = codes[i].des;
				vars_map[codes[i].des] = nodes_num++;
			}
		}
	...
	{
		for (int j = nodes_num; j > 0; j--)
		{
			do{
				if (dnodes[j].fathers_num == 0 && dnodes[j].enable)
				{
	...
	for (int i = stack_node_num - 1; i >= 0; i--)
	{
		...
		else
		{
			...
			else if (dnodes[output_stack[i].node_num].text == "OPP")
			{
				...
				|| dnodes[output_stack[i].node_num].text == "READ"
